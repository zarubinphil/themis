...
Принцип: кокпит НЕ меняет pipeline. Он только:
  1. принимает drag-drop документы → кладёт в $HOME/Desktop/inbox/ (Femida inbox)
...
#
# Секрет живёт в $HOME/.secrets/themis-panel.env (переменная THEMIS_PANEL_TOKEN);
...
PIPELINE = (
    ...
    "Шаг 1 — инбокс отдай агенту inbox-triage: он сам отберёт из $HOME/Desktop/inbox/ ТОЛЬКО группу "
