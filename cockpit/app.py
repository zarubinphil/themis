...
Все локально на этом Mac. Только localhost. Без секретов.
...
# Пока она слушает петлю на Маке владельца, ее защищает сама машина; как только
...
# Очередь реплик с темпом: claude отдает ассистент-блок из многих строк РАЗОМ.
# Если пушить все сразу — чат валится пачкой. Кладем в очередь и выпускаем по одной
# с паузой → диалог течет последовательно, как живой чат, между репликами «печатает…».
...
def _drain_pending() -> None:
    """Дождаться, пока очередь опустеет — финал идет последним."""
...
# Фемида — ведущая/оркестратор (не станция релея), свое лицо
...
_TECH = _re.compile(r"\.py\b|\.docx|\.md\b|create_docx|колонтитул|JUSTIFY|Times New Roman|00_intake|\\.agent/drafts|cases/|приложени[ея]\s*№|МЗ-|```|knowledge-map|practice\.md|positions\.md|\bWrite\b|директори|запис[ьи] файл|повторяю запис|сохран(ил|яю) файл|разреш[иь]|висят в очеред|в очеред|жму один раз|^\s*[-*]\s+[a-zа-я]|^#{1,6}\s|^\s*\d+\.\s"
    # статусы-маркеры шагов, токен-отчет, имена инструментов, служебка — НЕ речь:
    r"|^\s*\*{0,2}\s*✓?\s*Шаг\s*\d|Шаг\s*\d+\s*завершен|Отчет по токен|\bтокен"
    r"|^\s*\||SendMessage|Реестр агентов|хрупкая семерка|журнал дела|лог сесси"
...
# на каждом запросе. Изменил index.html → перезапусти сервер (start.command и так бьет порт).
...
# Статика стабильна по содержимому — даем браузеру кэшировать сутки.
...
def _client_name(folder: Path) -> str:
    ...
    if cm.exists():
        for ln in cm.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            if s.startswith("# "):
                title = s[2:].strip()
                # шаблонный H1 «Профиль: Фамилия И.О.» — берем часть после двоеточия
...
def _visible_docx(base: Path):
    ...
    Позитивный фильтр (этап 3, 19.08.2026): показываем `GOTOVO/`, а не «все, кроме
...
# ── Этап 8: то, куда ведет ссылка из Telegram ───────────────────────────────
...
def doc_by_id(id: str = ""):
    ...
    # Идентификатор пришел снаружи: путь из карты все равно обязан лежать внутри
    # дел — иначе подмененная карта откроет любой файл машины.
...
def voice_queue() -> JSONResponse:
    """Надиктованное боту. Расшифровка живет только здесь, внутри сети: в чат она
...
def miniapp() -> HTMLResponse:
    ...
    Живет ВНУТРИ приватной сети и закрыт тем же токеном, что и остальная панель.
...
# ── Общие куски промптов (DRY — раньше дублировались в трех эндпоинтах) ──────
# Правила живого диалога юристов в чате.
CHAT_RULES = (
    "Общение между агентами — скиллами /caveman + /humanizer: коротко, но живым "
    "человеческим языком (не робот, не рубленый телеграф), тепло и в характере, без эмодзи. "
    ...
    "звездочки для жирного (**текст**), решетки заголовков (#), горизонтальные черты (---), "
    "списки с маркерами, таблицы. Никаких отчетов по токенам и итоговых сводок в чат. "
    ...
    "направляет твердо, но почтительно, как достойных избранников. "
...
PIPELINE = (
    ...
    "исполняй трек, а не самый тяжелый вариант по умолчанию. "
    "Шаг 1 — инбокс отдай агенту inbox-triage: он сам отберет из $HOME/Desktop/inbox/ ТОЛЬКО группу "
    "файлов этого дела (окно mtime плюс тематическая сверка) и перенесет ее в 00_intake. "
...
def _redline_prompt(doc_path: str) -> str:
    return (
        "Ты — Фемида, богиня правосудия. Доверитель открыл готовый документ, внес СВОИ правки и "
...
def _emit_event(line: str) -> None:
    ...
    if t == "assistant":
        ...
        # иначе итог+таблица задваиваются. Финал дает pump после завершения процесса.
        return
    else:
        return
...
def _start_run(prompt: str) -> bool:
    ...
    def pump() -> None:
        ...
        RUN["active"] = False
        # новый/обновленный .docx → карточка документа в чат
        femida = PEOPLE_BY_ID["femida"]
...
# число файлов и на объем запроса перетащенная в окно папка забивала диск, а инбокс
# становился нечитаемым. Пороги вынесены в окружение: приемка обязана проверять
...
async def upload(files: list[UploadFile] = File(...)) -> JSONResponse:
    def otkaz(why: str) -> JSONResponse:
        ...
        # юрист видел половину дела и считал, что загрузил все.
...
async def stream(after: int = 0) -> StreamingResponse:
    ...
    async def gen():
        ...
        last_active = None
        while True:
            new = [m for m in list(MSGS) if m["id"] > cursor]  # снапшот — не рвется при del MSGS
            for m in new:
                cursor = m["id"]
...
def _bind_or_refuse(host: str) -> None:
    ...
    Выставленная на чужой интерфейс без токена, она отдает этот пульт любому,
    кто знает адрес. Отказ происходит ДО приема первого запроса, а не после
