...
| **api.telegram.org** | бот-пульт: уведомления владельцу и приём его команд | токен в `$HOME/.secrets/themis-telegram.env` (`THEMIS_TELEGRAM_BOT_TOKEN`), в конфиге и репозитории только ИМЯ переменной. Наружу уходят ТОЛЬКО даты, счёт и слово «готово»: ни ФИО, ни номера дела, ни суммы — держит сторож на выходе (`themis_bot.py --check-out`, поверх `pii_gate`). Документ не пересылается, вместо него ссылка внутрь приватной сети без токена. Отвечаем единственному `chat_id`. Опрос (long polling), входящий порт не открывается. Порядок настройки и отзыва — `knowledge/bot-protocol.md`. Бот персональный: каждый заводит своего в BotFather, пустой конфиг = бот выключен |
...
| Tavily MCP | зарегистрирован в `$HOME/.claude.json` без ключа; кроме того, MCP не наследуется агентом с явным списком `tools` — поручение агенту заведомо бесполезно (прецедент 02.08.2026: 249 950 токенов впустую) |
