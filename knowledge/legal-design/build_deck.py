...
def s_fonts(prs):
    """Утвержденная гарнитура. Слайд показывает ее собой же."""
    s = slide(prs)
    kicker(s, "Гарнитура")
    text(s, "Один шрифт, второго нет", M, Inches(1.15), CW, Inches(0.6), 30,
         DISPLAY, INK, bold=True)
    text(s, "PT Serif, решение владельца 04.08.2026 (отменяет набор из четырех "
            "гарнитур). Свободная лицензия SIL OFL, кириллица полная. "
            "Times New Roman выведен как проприетарный.",
         M, Inches(1.72), Inches(8.6), Inches(0.5), 13, BODY, GREY, spacing=1.25)

    # Роли различаются кеглем, начертанием и разрядкой — не сменой гарнитуры.
    rows = [
        ("PT Serif · разрядка", "PT Serif",
         "Титул: суд · заголовок · подзаголовок. Вразрядку 3 pt",
         "И С К О В О Е   З А Я В Л Е Н И Е"),
        ("PT Serif · прямое", "PT Serif",
         "Тело · цитаты · просительная · приложения · подпись",
         "Ответчик получил 6 000 000 рублей."),
        ("PT Serif · полужирное", "PT Serif",
         "Заголовки разделов · ПРОШУ · ПРИЛОЖЕНИЯ · шапки таблиц",
         "III. Расчет требований"),
        ("PT Serif · вправо", "PT Serif",
         "Числовые колонки — разряды держит выравнивание",
...
def s_checklist(prs):
    ...
    right = [
        ...
        "Гарнитура та же: PT Serif, одна на весь документ",
