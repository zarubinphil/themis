...
Пиши по-русски, официально-деловым стилем. Букву `е` не использовать.
...
- Роутер кладет рядом `<sha>.requisites.json` (ИНН/ОГРН/№ дела/суммы/даты/паспорт) — брать готовое. Смешанный PDF извлекается полностью (скан-страницы до-OCR-ятся).
...
- База «ДО»: `create_docx.py save()` авто-кладет снимок в `.agent/drafts/_baselines/<имя>.docx`.
