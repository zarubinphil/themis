...
Обновляется: `.Codex/`, `AGENTS.md`, `scripts/`, `cockpit/`, `bin/`, `install.sh`, `README.md`, `.mcp.json`, `docs/`; зависимости/пересборку OCR скрипт делает сам через `install.sh`. НЕ трогается: `cases/`, `knowledge/`, рантайм (кеши, `_baselines/`, `cockpit/.state.json`).
...
git status --porcelain -- .Codex AGENTS.md scripts cockpit bin install.sh .mcp.json docs
```
...
4. Exit ≠ 0 → ОТКАТ: `git checkout "$OLD" -- .Codex AGENTS.md scripts cockpit bin install.sh README.md .mcp.json docs` → доложить причину (последние строки вывода) и факт отката. СТОП. Нет сети → `git fetch` упадет до правок, откат не нужен.
