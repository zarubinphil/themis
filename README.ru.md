...
> Привет. Я Фил.
>
> Я сделал Фемиду для себя: надоело терять вечера на механику дела — двести страниц сканов, сверка дат, проверка ссылок. Если она пригодится и вам, я буду рад.
>
> Попробуйте. Что-то сломается — напишите в Issues, я их читаю. Понравится — поставьте звезду и расскажите знакомому юристу, который до сих пор делает это руками. И загляните в остальные проекты Олимпуса: https://zarubinvibe.ru
>
> — Филипп Зарубин
...
Три строки выше — это вся установка. `bash install.sh` ставит все сам и спрашивает разрешение перед каждым шагом. Никакой агент ему не нужен: хватает обычного терминала.

**Claude Code.** Запустите `claude` в этой папке и скажите `/themiz-setup`. Установка пройдет разговором, по одному вопросу.

**Codex CLI.** Запустите `codex` в той же папке. Те же агенты и те же правила уже лежат внутри проекта.

**VS Code или Cursor.** Откройте папку командой `code .` и запустите своего агента прямо в редакторе.

**Совсем без агента.** `python3 cockpit/app.py` откроет локальную панель на `http://127.0.0.1:8800`: там можно читать дело, следить за сроками и собирать документ руками.

Нет Git? Возьмите [ZIP](https://github.com/zarubinvibe/themiz/archive/refs/heads/main.zip) и распакуйте. Установка та же.
...
| Вариант | Что это | Где лежат материалы дела | Читает ваши сканы | Практика за вас и против | Готовит документ | Кто проверяет результат | Цена |
|---|---|---|---|---|---|---|---|
| **Фемида** | Многоагентный помощник по одному делу | На вашем Mac | Да, локально | Да, обе стороны | Да, по контракту дела | Второй агент, отдельная роль | Частному юристу бесплатно |
| Делать руками | Юрист и папка | У вас | Вы читаете сами | Сколько успеете за неделю | Да | Вы сами | Ваши часы |
| КонсультантПлюс, Гарант | Справочные правовые системы | Не для материалов дела | Нет | Поиск по нормам и практике | Шаблоны | Вы сами | Подписка |
| Судакт, картотека суда | Открытый поиск судебных актов | Не для материалов дела | Нет | Поиск актов | Нет | Вы сами | Бесплатно |
| ChatGPT, Claude как есть | Общий чат-помощник | В облаке поставщика | Если приложите файл | По памяти модели | Да | Вы сами | Подписка |
| Harvey, CoCounsel | Юридический AI для фирм | В облаке поставщика | Да | Да, по своим юрисдикциям | Да | Зависит от тарифа | Корпоративный договор |
| Doczilla, FreshDoc | Конструкторы документов | В облаке поставщика | Нет | Нет | Да, по шаблону | Вы сами | Подписка |
