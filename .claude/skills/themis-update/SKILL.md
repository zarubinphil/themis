...
Обновляется: `.claude/`, `.codex/`, `.agents/`, `AGENTS.md`, `scripts/`, `cockpit/`, `bin/`, `install.sh`, `README.md`, `docs/`; зависимости/пересборку OCR скрипт делает сам через `install.sh`. НЕ трогается: `cases/`, `knowledge/`, рантайм (кеши, `_baselines/`, `cockpit/.state.json`).
...
git status --porcelain -- .claude .codex .agents AGENTS.md scripts cockpit bin install.sh docs
```
...
4. Exit ≠ 0 → ОТКАТ: `git checkout "$OLD" -- .claude .codex .agents AGENTS.md scripts cockpit bin install.sh README.md docs` → доложить причину (последние строки вывода) и факт отката. СТОП. Нет сети → `git fetch` упадет до правок, откат не нужен.
