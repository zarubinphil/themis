...
Обновляется: `.claude/`, `AGENTS.md`, `scripts/`, `cockpit/`, `bin/`, `install.sh`, `README.md`, `.mcp.json`, `docs/`; зависимости/пересборку OCR скрипт делает сам через `install.sh`. НЕ трогается: `cases/`, `knowledge/`, рантайм (кеши, `_baselines/`, `cockpit/.state.json`).
...
4. Exit ≠ 0 → ОТКАТ: `git checkout "$OLD" -- .claude AGENTS.md scripts cockpit bin install.sh README.md .mcp.json docs` → доложить причину (последние строки вывода) и факт отката. СТОП. Нет сети → `git fetch` упадет до правок, откат не нужен.
