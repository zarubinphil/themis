...
### Шаг 8. Раунд Кони по .md — ДО сборки
...
материалов) + путь к делу + применённые redlines; Кони не перечитывает
справочники и кеши. До 2 раундов:
- «ГОТОВ К ПОДАЧЕ» → записать вердикт и идти на Шаг 9:
  `python3 scripts/verdict.py ЧЕРНОВИК.md --record --verdict 'ГОТОВ К ПОДАЧЕ' -r N`
- «ТРЕБУЕТ ПРАВОК» → внести → `_v2.md` → повторный раунд (вердикт отзывается,
  сборка снова закрыта);
...
b.save("cases/.../.agent/drafts/{документ}_v1.docx")   # авто: «ё»→«е», baseline в _baselines/
```
...
| Артефакт | Путь | Когда |
|---|---|---|
| Черновик | `.agent/drafts/{документ}_v1.md` | всегда |
| Word | `.agent/drafts/{документ}_v1.docx` (DocBuilder) | всегда |
| Снимок ДО | `.agent/drafts/_baselines/{документ}_v1.docx` (кладет `save()`, проверить) | всегда |
...
7. `.md` + `.docx` в `.agent/drafts/`, снимок в `_baselines/`.
