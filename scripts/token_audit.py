...
Зачем. token_ledger считает деньги по шагам, и его цифра идёт в бюджетные гейты.
Но кто проверяет сам ledger? Одна реализация, ошибись она в дедупликации или
тарифе, врёт молча и уверенно. Этот прибор — второй, НАМЕРЕННО отдельный счётчик:
...
весь смысл прибора. Импортируй мы счётчик ledger — сверяли бы его с самим собой,
и общий баг остался бы невидимым. Дублирование логики подсчёта тут не нарушение
...
# те же, что у token_ledger: сверяем СПОСОБ подсчёта, а не выдумываем свой прайс.
RATES = {
...
def scan_files(files: list[str]) -> tuple[int, float]:
    ...
    строки и строки без usage не в счёт.
    """
    total = 0
    money = 0.0
    for path in files:
...
def cmd_compare(cwd: str, tol: float) -> int:
    ...
    # не касается и остаётся независимым счётчиком.
    import token_ledger
    ...
    print(f"token_audit: расхождение больше допуска {tol*100:.0f}% — баг в одном из "
          "счётчиков, разобраться", file=sys.stderr)
    return 1
...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        checks += [
            ("дубль по requestId схлопнут, subagent учтён", tot == 100 + 10000),
    ...
    if bad:
        ...
        return 1
    print(f"selftest пройден: {len(checks)}/{len(checks)} — свой счётчик без сети")
    return 0


def main() -> int:
    ap = argparse.ArgumentParser(description="Независимая сверка расхода токенов")
    ap.add_argument("--json", action="store_true", help="свой подсчёт: {total, money}")
