...
def _text_bez_polya(p_el):
    """Текст абзаца колонтитула БЕЗ поля номера страницы.

    Служебно — само ПОЛЕ (w:fldSimple / пара w:fldChar…w:fldChar с
    w:instrText), а не абзац целиком: требование, подсаженное в абзац
    колонтитула с полем PAGE, уходило из-под сверки с одобренной редакцией
    вместе с полем и стояло внизу каждой страницы поданного документа
    (этап 9.22, круг 9). Кэшированное значение поля («2») — часть поля и
    вычитается с ним; остальной текст абзаца возвращается и судится.
    """
    out = []

    def walk(el):
        if el.tag == qn("w:fldSimple"):
            return                       # поле целиком, со значением-кэшем
        if el.tag == qn("w:r"):
            if el.find(qn("w:fldChar")) is not None \
                    or el.find(qn("w:instrText")) is not None:
                return                   # ран сложного поля
        if el.tag == qn("w:t"):
...
class DocBuilder:
    ...
    def __init__(self):
        ...
        # с одобренной редакцией (_matches_approved) их не судит, все остальное
    ...
    def add_image(self, path, caption=None, width_mm=155):
        ...
        (L3) составляет 160 мм, запас в 5 мм не дает Word ужать картинку
    ...
    def add_table(self, headers, rows, widths=None, font_size=12,
        ...
        def _fill(cell, value, col_idx, bold_default=False):
            ...
            # Гарнитура одна на все (решение владельца 04.08.2026). Роли
    ...
    def add_addressee_table(self, blocks):
        ...
        создает служебных строк суда и дела.
    ...
    def _strip_yo(self):
        """Заменяет е→е, Е→Е во всех текстовых узлах (правило проекта «нет е»)."""
    ...
    def _content_units(self):
        ...
        служебного поля номера страницы. Служебно — ПОЛЕ, а не абзац:
        требование, подсаженное в абзац колонтитула с полем PAGE, уезжало
        вместе с полем (этап 9.22, круг 9) — из служебного абзаца вычитается
        только текст поля, остаток судится. Читаются по XML существующих
        part'ов, чтобы доступ не создавал пустых колонтитулов в сохраняемом
        файле.
        """
        units = []
        ...
        for part in self.doc.part.package.iter_parts():
            ...
            for p in root.iter(qn("w:p")):
                if p in service:
                    # Абзац с полем номера страницы: вычитаем ТОЛЬКО текст
                    # поля, остаток — обычный текст и судится наравне.
                    text = _text_bez_polya(p)
                    if text.strip():
    ...
    def save(self, path):
        ...
        # Точка сборки одна: готовый документ живет в GOTOVO/, снимок — рядом с
        ...
        # конвейера (тесты, разовые сборки) остается здесь как последний рубеж.
        if not is_case_doc:
