...
порядок свойств), которых доверитель не касался. Идём через объектную модель:
берём то, что человек реально менял — текст абзацев, гарнитуру, поля, выравнивание.
"""
...
def format_diff(before: dict, after: dict) -> list[dict]:
    ...
    # выравнивание» неотличимо от «другой абзац» и учить на нём нечего.
...
def selftest() -> int:
    ...
    checks = [
        ...
        ("удалённый текст виден в разборе", any("удалит" in e["было"] for e in c)),
