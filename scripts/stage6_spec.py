#!/usr/bin/env python3
"""stage6_spec.py — приемка этапа 6 «сервер». Пишет КООРДИНАТОР, не исполнитель.

Инвариант роя: generator ≠ verifier. Контракт задан снаружи и проверяется черным
...
владелец. Приемка обязана доказать, что система безопасна ДО того, как ее выставят.
...
Пять работ:
  ...
  5. приемник синхронизации берет только извлеченный текст: оригинал, бинарник,
...
# С ВЕРНЫМ токеном дергаем только те маршруты, что не запускают конвейер: проба
# аутентификации не должна поднимать настоящий прогон Claude по делам владельца
# (первый же вызов /api/task на старте приемки его запустил — снято вручную).
...
AUTH_CONTRACT = """  cockpit/app.py — аутентификация
    ...
    Открытие панели по ссылке /login?token=… ставит cookie, дальше браузер носит ее сам:
    юрист не должен вводить заголовки руками.
    Статика (/, картинки) остается открытой — иначе панель не загрузится, чтобы
    спросить токен."""
...
# ── 5. Приемник синхронизации ───────────────────────────────────────────────
SYNC_CONTRACT = """  scripts/sync_receiver.py — приемник синхронизации Mac → сервер
    Решение владельца: на сервер уходит только ИЗВЛЕЧЕННЫЙ ТЕКСТ, оригиналы — никогда.
    ...
    Принимается: .md/.txt/.json с текстом. --selftest дает 0 без сети."""
...
SECRET_CONTRACT = """  Секрет и протокол сервера
    ...
    knowledge/server-protocol.md существует и называет поименно: отдельного
...
CHECKS = [
    ...
    ("приемник берет только текст", check_sync, SYNC_CONTRACT),
...
def selftest():
    ...
    try:
        ...
        SCRIPTS = saved
    print("selftest: приемка краснеет на отсутствующем приборе — ок")
    return 0


def main():
    ap = argparse.ArgumentParser(description="Приемка этапа 6 (пишет координатор).")
