...
PII_CONTRACT = """  scripts/pii_gate.py
    ...
    --residual ВХОД
        ...
        данные. Нужен как второй рубеж: маскировщик может не узнать форму,
        и тогда решение «отпускать наружу» обязано принимать не он же.
...
def check_pii():
    ...
    with tempfile.TemporaryDirectory(prefix="stage5-pii-") as tmp:
        ...
        # Второй рубеж: --residual
        code, out, err = run([tool(name), "--residual", str(src)])
        if code != 1:
            fails.append((name, f"--residual на тексте с реквизитами вернул {code}, ожидался 1"))
        clean_probe = t / "residual_clean.md"
        clean_probe.write_text("Обзор практики по неустойке.\n", encoding="utf-8")
        code, out, err = run([tool(name), "--residual", str(clean_probe)])
        if code != 0:
            fails.append((name, f"--residual на чистом тексте вернул {code}, ожидался 0"))
    return fails
