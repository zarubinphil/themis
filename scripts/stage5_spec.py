#!/usr/bin/env python3
"""stage5_spec.py — приемка приборов этапа 5. Пишется КООРДИНАТОРОМ, не исполнителем.

Инвариант роя: generator ≠ verifier. Если рой пишет и прибор, и его `--selftest`,
то «selftest зеленый» не доказывает ничего — исполнитель подгонит проверку под то,
что получилось. Поэтому контракт каждого прибора задан здесь, снаружи, и проверяется
черным ящиком: только через командную строку, без импорта потрохов.

Роли этот файл НЕ ПРАВЯТ. Прибор обязан подстроиться под приемку, не наоборот.

Сеть при приемке заглушена подставным прокси: прибор, полезший в сеть на `--selftest`,
упрется в отказ соединения. «Работает без сети» — проверяемое утверждение, а не обещание.
...
# Любая попытка выйти в сеть упрется в закрытый порт и упадет быстро.
...
def tool(name):
    ...
    и относительный путь молча увел бы запуск обратно в боевой scripts/."""
...
def selftest_clean(name):
    """Общее требование ко всем приборам: `--selftest` дает 0 и не ходит в сеть."""
...
PII_CONTRACT = """  scripts/pii_gate.py
    --mask ВХОД --out ВЫХОД --map КАРТА.json
        ...
        Ничего не нашел → код 1 и ВЫХОД НЕ СОЗДАЕТСЯ (fail-closed: пустая карта
        значит «не сработало», а не «чисто»).

        МАСКИРУЕТСЯ (первая редакция контракта требовала только полного ФИО и
        реквизитов с меткой — прибор по ней тек на 10 пробах из 11):
          · фамилии доверителей из реестра дел — БЕЗУСЛОВНО, в любой форме и падеже.
            Список берется с диска: scripts/pd_guard.py, функция client_names().
            Это ровно тот набор имен, ради которого гейт существует;
...
def check_pii():
    ...
    with tempfile.TemporaryDirectory(prefix="stage5-pii-") as tmp:
        ...
        # Формы, на которых прибор первой редакции потек
        ...
        for title, text, keep in PII_KEEP:
            ...
            if f_out.is_file() and keep not in f_out.read_text(encoding="utf-8"):
                fails.append((name, f"лишняя маскировка ({title}): {keep!r} затерт, "
        ...
        residual_cases = [
            ...
            # Падежи: морфология по СТЕМУ, а не по одной форме. «Сидоровым»
            # кончается на «ым», и список окончаний именительного его не ловит.
            ("фамилии в творительном падеже",
             "Спор между Сидоровым и Ковалевым.\n", 1),
            ("фамилия в дательном", "Передать Никитиной оригиналы.\n", 1),
            # Телефон без кода страны — обычная запись в материалах дела
            ("телефон в скобках без кода страны",
             "Телефон (917) 123-45-67 доверителя.\n", 1),
            ...
            # склонение, то есть фамильная морфология по форме — а стоит эта
            # фраза почти в каждом документе. Ложная тревога здесь означает,
            # что рубеж блокирует все и им перестанут пользоваться.
            ("Конституционный Суд",
             "Конституционный Суд Российской Федерации указал.\n", 0),
            ("Верховный Суд",
             "Верховный Суд Российской Федерации разъяснил порядок.\n", 0),
            ("кодекс с наименованием государства",
             "В соответствии с Гражданским кодексом Российской Федерации.\n", 0),
            ("наименование государства и город",
...
def check_audit():
    ...
    if "token_ledger" in src and "--compare" not in src:
        fails.append((name, "считает через token_ledger — это не независимая сверка, "
                            "а ее пересказ; свой проход по session-JSONL обязателен"))
    return fails


# ── 4. superseded_by в логе уроков ──────────────────────────────────────────
LESSONS_CONTRACT = """  scripts/lessons_supersede.py
    --check   код 1, если урок отменен более новым, но не помечен `superseded_by`
...
def selftest():
    """Проверяет саму приемку: заглушка сети стоит, отсутствующий прибор виден,
    ...
    with tempfile.TemporaryDirectory(prefix="stage5-spec-selftest-") as tmp:
        ...
        try:
            ...
            assert fails, "подставной обезличиватель принят приемкой"
...
def main():
    ap = argparse.ArgumentParser(description="Приемка приборов этапа 5 (пишет координатор).")
