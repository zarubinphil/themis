...
ЧТО СЧИТАЕТСЯ. Два рубежа считаются РАЗДЕЛЬНО, и это принципиально:
  ...
  · имена доверителей (`pd_guard.name_pattern` с диска) — ради них все и
  ...
  · шаблоны второго рубежа (паспорт/СНИЛС/кадастр/госномер/дата рождения) —
    эвристика с намеренно избыточной маскировкой, она дает ложные тревоги на
...
# Список ведется руками: молчаливое расширение превратило бы гейт в украшение.
RAZOBRANO = {
...
def audit(repo: str, ref: str = "HEAD") -> dict:
    ...
    if not pat_lat:
        # Пустой шаблон означает «имен не нашлось на диске», а не «все чисто».
        # Тот же fail-closed, что в pii_gate: молчаливый ноль опаснее отказа.
        return {"ok": False, "prichina": "пустой шаблон имен — fail-closed",
...
def report(r: dict) -> int:
    ...
    if r["nerazobrano"]:
        ...
        return 1
    print("✓ ЧИСТО: имен доверителей в истории нет; второй рубеж — только разобранные")
    return 0
...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        Path(td, "a.md").write_text("обычный текст без имен\n", encoding="utf-8")
        gg(td, "add", "-A"); gg(td, "commit", "-qm", "чисто")
        proverki.append(("чистая история — зеленый", audit(td)["ok"] is True))

        # имя живет ТОЛЬКО в промежуточном коммите и убрано следующим —
