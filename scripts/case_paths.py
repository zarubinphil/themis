...
Документ живёт в `.agent/drafts/<имя>.md` весь цикл правок. `.docx` собирается
ОДИН раз — после вердикта Кони «ГОТОВ К ПОДАЧЕ» — и кладётся в `GOTOVO/`.
...
# Что человеку видно в корне дела. Всё прочее — кухня.
...
def nfc(name):
    ...
    Непереведённое имя на сервере превращается в другой путь, и дело просто
    не находится. На диске уже есть дело с турецкой «ı» — на нём это видно.
    """
