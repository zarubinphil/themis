...
Движок берётся в таком порядке: переменная THEMIS_STT_CMD → `voice.stt_cmd` из конфига
...
Модель whisper подкачивается из сети при первом запуске, поэтому её отсутствие — тоже
отказ, а не «сейчас докачаю»: докачка и есть то самое обращение наружу, которого здесь
быть не должно. Модель кладёт владелец заранее.
...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            assert "THEMIS_PANEL_TOKEN" not in _sreda(), "секрет ушёл бы движку в окружение"
        finally:
            os.environ.clear()
