...
def snils_valid(snils: str) -> bool:
    ...
    ДЫРА, найденная аудитом 03.08.2026: окно «контроль не определен» проглатывало
...
# подходят 182 записи из 1429 (12,7 %), и жесткая проверка на «04» отвергала бы
# реальные реквизиты казначейства — а именно они стоят в платежках по госпошлине.
...
def bik_valid(bik: str) -> bool:
    ...
    банка дает DaData по ключу `bank` (см. knowledge/allowed-services.md), а здесь
...
def scan_requisites(req: dict, bik: str | None = None) -> list[str]:
    ...
    # роутер кладет в <sha>.requisites.json, и до 04.08.2026 scan_requisites его
    ...
    # Счет ключуется БИК-ом. В одном документе (выписка, договор, платежка) БИКов
    # обычно несколько — свой у каждого банка, и какой счет к какому относится, из
    # плоского списка реквизитов не видно. Поэтому счет считается подтвержденным,
    # если он ключуется ХОТЬ С ОДНИМ БИКом документа; замечание — только когда ни
    # один не подошел. Перебор «каждый счет против каждого БИКа» давал ложную
...
def _selftest():
    ok = [
        ...
        # СНИЛС: окно «контроль не определен» глотало пустышку до 04.08.2026.
        ...
        # Счет ключуется БИК-ом из самого документа, без явного аргумента.
        scan_requisites({"bik": ["042202824"],
                         "account": ["40702810029160002367"]}) == [],
        len(scan_requisites({"bik": ["042202824"],
                             "account": ["40702810029160002368"]})) == 1,
        # Несколько банков в одном документе: счет ключуется своим БИКом, чужой
