...
# Короткая строка (шапка, подпись, номер листа) совпадает по случайности и даёт
...
def check_md_vs_docx(md_path: str, docx_path: str) -> list[str]:
    ...
    # Объём и числа рассогласование ТЕКСТА не ловят. Прецедент 21.08.2026: .md
    # правился по замечаниям рецензента, .docx остался прежним — четыре внесённых
    # правки в собранный документ не попали, а сторож дал код 0 и «согласованность
    # в порядке». Объём изменился на доли процента, крупные числа не тронуты —
    ...
    # (21.08.2026, четыре лишних круга к рецензенту, составитель обошёл сторожа
    # короткой шапкой). Правка же, ради которой проверка и заводилась, живёт
    # в .md и обязана доехать до .docx — это направление её и ловит, а служебные
