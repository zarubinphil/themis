...
обратно приходит текст, на диск пишет Claude через наши ворота.**
    ...
    --role РОЛЬ --prompt ФАЙЛ [--registry ФАЙЛ] [--cache ФАЙЛ] [--timeout СЕК]
              [--out ФАЙЛ] [--log ФАЙЛ]
    --selftest

Исполнитель назначает реестр по роли (cli_router). Свободных --provider/--cmd
из командной строки нет: прямая команда мимо реестра — обход всего контура
(ни обезличивания, ни класса данных, ни пробы), а материалы дела — тайна.
...
def main() -> int:
    ...
    # --provider/--cmd исполнению из CLI не подлежат: исполнитель берётся ТОЛЬКО из
    # реестра по --role. Свободная команда — обход всего контура: чужой бинарник
    # получает материалы дела без обезличивания, класса данных и пробы (проба
    # 20.08.2026 провела бинарник мимо ворот, файл-маркер записался). Флаги
    # оставлены объявленными ради старого шва, но их приход без --role — фейл-клоуз.
    # Прямой доступ к call() остаётся у selftest этапа 7 — он идёт мимо argparse.
    if a.provider or a.cmd:
        return _otkaz("свободные --provider/--cmd исполнению не подлежат — исполнитель "
                      "назначает реестр по --role; прямая команда мимо реестра уносит "
                      "материалы дела без обезличивания и пробы (ст. 8 ФЗ № 63-ФЗ)")
    ap.error("нужна --role, либо --selftest")
