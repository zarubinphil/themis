#!/usr/bin/env python3
"""lessons_supersede.py — стеречь отмененные уроки в журнале самообучения.

Зачем. `knowledge/lessons-log.md` копит уроки годами. Когда новый урок отменяет
старый (сменился стандарт, отклонен прежний движок, развернуто прежнее решение),
старая запись остается в файле как ни в чем не бывало. А устаревшая конвенция
ХУЖЕ отсутствующей: будущий агент читает ее сверху вниз и уверенно исполняет
мертвое правило (прецедент: конституция год держала движком OCR то, что пилот уже
отклонил). Пометка `superseded_by` на старой записи гасит этот риск — читатель
сразу видит, что урок снят и чем заменен.

Что именно ловит `--check`. Только ОБЪЯВЛЕННУЮ, но не проведенную отмену: новый
...
def mark_text(text: str, old: str, new: str) -> tuple[str, int]:
    """Проставить `superseded_by` под каждым заголовком с датой old (еще не помеченным)."""
    changed = 0
...
def cmd_check() -> int:
    ...
    if not findings:
        print("lessons_supersede: отмененных без пометки не найдено — чисто")
        return 0
    print("lessons_supersede: уроки отменены более новыми, но без `superseded_by`:")
    for old, new in findings:
        print(f"  {old} отменен уроком {new} → пометить: "
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Стеречь отмененные уроки в журнале самообучения")
