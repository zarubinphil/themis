#!/usr/bin/env python3
"""bot_avatar.py — аватар бота Фемиды: весы правосудия на темном поле.

Зачем прибором, а не картинкой в репозитории. Растр под `cases/` запрещен отдельным
...
одинаковый PNG, поэтому «обновил аватар» видно диффом пути, а не размером блоба.
    ...
    --out ФАЙЛ [--size N]   кладет PNG (по умолчанию 512×512) и печатает путь
...
def main() -> int:
    ...
    # Под cases/ растр запрещен отдельным правилом проекта. Правило держит хук
    # Claude, но хук стоит на инструменте, а не на цели записи: скрипт, запущенный
    # руками, прошел бы мимо него. Гейт ставится на цель.
