...
а не факт; адрес чата берется только из `message.chat.id`, который проставил Telegram.
...
_NUM = r"\d[\d \u00a0\u202f']*(?:[.,]\d{1,2})?"
_CUR = r"(?:руб(?:\.|лей|ля|ль)?|р\.|₽|коп\.|копе\w*|RUB|USD|EUR|доллар\w*|евро)"
_MAG = r"(?:тыс\.?|млн\.?|млрд\.?)"
_MONEY_WORD = (
    ...
    r"двести|триста|четыреста|пятьсот|шестьсот|семьсот|восемьсот|девятьсот|"
    r"тысяч\w*|миллион\w*|миллиард\w*"
)
DENGI = re.compile(
    rf"{_NUM}\s*(?:{_MAG}\s*)?(?:{_CUR})"
    rf"|(?:{_CUR})\s*{_NUM}"
    rf"|{_NUM}\s*(?:{_MAG})\b"
    rf"|(?:{_MONEY_WORD})(?:\s+(?:{_MONEY_WORD})){{0,8}}\s+(?:{_CUR})",
    re.I,
)
...
def bez_sekreta(s: str, c: dict) -> str:
    """Ни одна строка наружу не несет значение токена — ни в жалобе, ни в журнале."""
    t = token(c)
...
def gotovnost(c: dict, path: str | None = None) -> list:
    ...
    if not svoy:
        ...
        # сравнивает строки, просто ни одна не совпадет никогда.
...
def audit(sobytie: str, chat: str = "", detal: str = "") -> None:
    ...
    # отказов меняются, и однажды в деталь затечет строка с адресом запроса.
...
def state_path() -> Path:
    """Смещение опроса лежит рядом с журналом: у приемки и у сервера свои каталоги."""
...
def chisto(text: str) -> list:
    ...
    так же выдает дело, а pii_gate ею не занимается."""
...
# ── Речь ────────────────────────────────────────────────────────────────────
# Худший случай для приемки: шаблонам подается дело со ВСЕМИ реквизитами. Ни одно
...
# счет и ссылка на панель. Ничего из этого не указывает на доверителя.
SAFE = {
    "data": "21.08.2026",
    "vremya": "10:00",
    "dney": 3,
...
def shablony(d: dict) -> dict:
    ...
    return {
        "start": (
            ...
            "Сразу про главное: этот чат лежит на серверах Telegram, и все "
            "написанное здесь им видно. Поэтому имен, номеров дел и сумм я тут "
            "не называю — только даты, счет и «готово».\n\n"
            ...
            "панель. Можно голосом: расшифрую здесь, на машине, звук никуда не уйдет."
        ),
...
def doc_id(put: str) -> str:
    ...
    в ссылку, которая уйдет в Telegram, это попадать не должно. Соль своя у каждой
    установки: без нее идентификатор перебирается по списку фамилий."""
    d = _links()
    sol = d.get("salt")
    if not sol:
        sol = os.urandom(16).hex()
        d["salt"] = sol
    # Только буквы, без цифр: шестнадцатеричный идентификатор с шансом дает десять
    # цифр подряд, а это форма ИНН — сторож на выходе честно принял бы такую ссылку
    # за реквизит и не выпустил ее. Буквенная запись эту коллизию исключает.
    ...
    try:
        ...
        raise Otkaz(f"карта ссылок не пишется ({p}): {e.strerror}. "
                    "Без нее панель не узнает, какой документ отдавать.")
    try:
        p.chmod(0o600)        # карта ссылок называет дела — читает ее только владелец
    except OSError:
        pass
    return ident
...
def baza(c: dict) -> str:
    ...
    if not url:
        raise Otkaz("адрес панели не задан (server.url) — ссылке некуда вести. "
                    "Пока панель только на этой машине, документы забираются с нее напрямую.")
    return url


def _v_delakh(put: str) -> bool:
    """Путь ведет внутрь cases/. Проверяется ЗДЕСЬ, а не только у панели: карта
    ссылок иначе копит записи на «../../etc/passwd», и любой ее недосмотр
...
def doc_link(c: dict, put: str) -> str:
    # Токен в ссылку НЕ кладем: отправить его в Telegram значит его разгласить.
    # Владельца узнает панель — по своему входу, на своей стороне.
    if not _v_delakh(put):
        raise Otkaz("ссылку даю только на документ внутри дел (cases/) — "
                    f"путь «{put}» ведет наружу")
...
def _pismo(chat: str, text: str) -> dict:
    ...
    Telegram идет по ней САМ, чтобы собрать карточку. Наш адрес ведет внутрь
...
def _blizhaishee(dela: list) -> str:
    """Ближайшее событие — датой. Имя события наружу не идет: в нем бывает суть спора."""
...
def golos(c: dict, api_base: str, file_id: str) -> str:
    ...
    остается на машине: в надиктовке звучат фамилии и суммы, и наружу она не уходит —
...
def once(c: dict, api_base: str, dolgiy: bool = False) -> int:
    ...
    try:
        ...
        offset = 0
    # Долгий опрос: держим соединение сами и ждем ответа Telegram, а не дергаем
    # его в цикле. Свой таймаут заведомо больше — иначе рвем собственный запрос.
    ...
    for u in updates:
        ...
        # остается заявлением: им и подделывают доступ.
...
def _vzyat_zamok() -> bool:
    ...
    try:
        ...
        try:
            ...
            # Замок от процесса, которого больше нет: снимаем и берем себе.
            try:
                z.unlink()
            except OSError:
                return False
...
def serve(c: dict, api_base: str, cycles: int | None = None) -> int:
    ...
    механизм, но день бот живет здесь. Порт наружу не открывается: инициатива
    всегда наша."""
    if not _vzyat_zamok():
        print(f"ОТКАЗ: бот уже опрашивает Telegram (замок {_zamok()}). "
              "Два опрашивающих делят сообщения между собой — половина ответов пропадет.",
              file=sys.stderr)
        return 1
...
def svodka_sobytiy(dney: int = 1) -> str:
    ...
    if len(unikalnye) > len(pokazat):
        # Перечислять все нельзя: длинное уведомление не уйдет вовсе (предел
        # Telegram), и владелец останется вообще без напоминания.
        kogda += f" и еще дней с заседаниями: {len(unikalnye) - len(pokazat)}"
...
def cmd_check_out(src: str) -> int:
    ...
    beda = chisto(text)
    if beda:
        print("наружу не уйдет: " + ", ".join(beda), file=sys.stderr)
        return 1
    print("чисто — такой текст бот отправит")
    return 0


PREDEL = 4096          # жесткий предел Telegram на одно сообщение
...
def cmd_chat_probe(c: dict, api_base: str) -> int:
    ...
    Печатает номера чатов и ничего не отвечает — на этом шаге еще некому."""
...
def cmd_notify_doc(c: dict, api_base: str, put: str) -> int:
    """Документ готов — одно сообщение со ссылкой внутрь сети. Зовется, когда
    составитель закончил: имя документа и дело в текст не попадают, ссылка
    непрозрачна, забирается все на панели."""
...
def cmd_notify_hearing(c: dict, api_base: str, data: str, vremya: str, put: str) -> int:
    """Заседание и собранная к нему папка — одно сообщение. Зовется, когда готов
...
def cmd_notify_deadline(c: dict, api_base: str, data: str) -> int:
    ...
    и называет только ее: какой именно срок и по какому делу, видно в панели."""
    try:
        d, m, g = data.split(".")
...
def selftest() -> int:
    ...
    # Шаблон без вызывающего — мертвая речь: приемка его проверяет, а владелец
    ...
    for imya in sh:
        ...
        assert vne_opredeleniya > 0, \
            f"шаблон «{imya}» ничем не отправляется — мертвая речь"
    assert chisto("Иванова Мария Петровна ждет документ"), "сторож слеп к ФИО"
    ...
    assert not chisto("Дел в работе: 12. Ближайшее заседание 21.08.2026."), \
        "ложная тревога на счете и дате"
    ...
    assert _pismo("1", "текст")["disable_web_page_preview"] is True, \
        "превью ссылки включено — обходчик Telegram пойдет по нашему адресу сам"
