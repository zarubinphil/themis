...
# Интерпретатор выбирается явно: launchd не дает логин-шелл, и `python3`
# из системного PATH — это 3.9, на которой падает импорт (прецедент 21.08.2026).

# Корень репозитория берем от расположения самого скрипта, а не жесткой
...
# Единственный внешний канал уведомлений. Наружу уходят только даты и счет:
