...
часто через OCR, который путает цифры и разделители. Прежде чем номер попадет в
карту дела и в требования, дешевая локальная проверка отсекает заведомо
...
цифра — у кадастрового номера ее нет. «Контроль» здесь структурный: формат
...
не ноль. Номер, нарушивший эту структуру, заведомо неверен; прошедший ее —
...
def plausible(number: str) -> tuple[bool, str]:
    ...
    if int(m.group(4)) == 0:
        return False, "номер объекта не может быть нулем"
    return True, ""
...
def main() -> int:
    ...
    if a.selftest:
        return selftest()
    if a.check is not None:  # именно is not None: пустая строка — валидный вход, ее тоже проверяем
