...
фамилия человека.** Коммит ушел фоновым auto-sync в публичный репозиторий, и
...
Опубликованный коммит индексируется и остается в форках и зеркалах даже после
...
ПРИМЕНЕНИЕ:
    ...
    python3 scripts/pd_guard.py --tree             # все, что уже под контролем git
...
def _worktree_cases_dirs(root: str = ROOT) -> list[str]:
    ...
    Берем имена из основного дерева, не из git-индекса. Это держит один и тот же
...
# ее могут написать кириллицей — «Тестфама» вместо «testfam-ab».
_LAT2CYR = (
...
def _owner_latin_stems() -> set[str]:
    ...
    Источник — адрес `origin`, а не отдельный список: список разошелся бы с
...
def name_pattern(names: list[str], cyrillic: bool = False,
    ...
    дереву за прогон 19.08.2026), а сторож с ложной тревогой на обиходе не живет."""
    if not names:
        return None
    owner = _owner_stems()
    ...
    for n in sorted(names, key=len, reverse=True):
        ...
        # Стем короче 5 букв в шаблон не идет: «sud»→«суд» с хвостом [а-яё]{0,3}
        # ловил «суда», «судом», «судебн» — 97 ложных тревог по дереву за один
        # прогон (19.08.2026), а сторож с ложной тревогой на обиходе не живет.
        fam = parts[0]
    ...
    # несет хвост: «testfam-ab-2026.zip», «session-testfam-ab-19-08.md», «testfam-ab2».
    ...
    # Буква слева/справа по-прежнему рвет совпадение — «xfamiliya-abx» и
...
# задачи — обезличивания извлеченного текста дела перед отправкой наружу, где
...
def scan_pii(text: str, where: str) -> list[str]:
    ...
    без метки папки дела. pii_gate живет своей жизнью (стадии 6/7), здесь его
...
def _is_test_fixture_code(path: str) -> bool:
    ...
    штатно несет СИНТЕТИЧЕСКИЕ примеры нужной формы («СНИЛС 123-456-789 64») —
    именно такими фикстурами код и проверяют. Реальная утечка ПД идет другим
    каналом (имя папки дела в прозе/пути — `name_pattern` его ловит независимо
    от расширения файла); `scripts/` клиентских данных не несет по правилам
...
def check_ref_txn(pat: re.Pattern | None, state: str, stdin: str | None = None) -> list[str]:
    ...
    Тег создается ЛОКАЛЬНО и уезжает при push, а git-хука на создание тега нет.
...
def _push_files(local_sha: str, remote_sha: str) -> list[str]:
    ...
    if remote_sha and not _ZERO_SHA_RE.match(remote_sha):
        ...
        if base:
            # Удаленные пути не несут содержимого: их blob в уходящем коммите
            # не существует, и fail-closed на нем — ложная тревога.
...
def local_log_files(root: str = ROOT) -> list[str]:
    """Рабочие логи, где имена дел лежат законно: корневые *.log и все под cases/_logs/."""
...
def report(problems: list[str], what: str) -> int:
    ...
    if len(problems) > 20:
        print(f"   … и еще {len(problems) - 20}", file=sys.stderr)
    print("\nИмя папки дела — это фамилия человека. Репозиторий публичный, а "
          "опубликованный коммит остается в форках и зеркалах после удаления.\n"
...
def main() -> int:
    ...
    ap.add_argument("--tree", action="store_true", help="проверить все дерево git")
...
def selftest() -> int:
    ...
    checks = [
        ...
        ("слишком короткое имя не берется", "ab" not in names),
        # Ровно тот случай, который и произошел 04.08.2026.
        ...
        ("фамилия в сообщении коммита ловится",
         len(scan_text("fix: развел familiya-ab и ее двойника", pat, "msg")) == 1),
        ...
        # начнет краснеть на ровном месте и его выключат.
        ...
        ("пустой список имен никого не ловит",
        ...
        ("отчет по находкам дает код 1", report(["x"], "тесте") == 1),
        ("отчет без находок дает код 0", report([], "тесте") == 0),
        ...
        ("scripts/*.py опознается как тестовый код",
        ...
        ("чистый .docx без имен проходит", scan_text(clean_docx, pat, "hod.docx") == []),
        ...
        # репозитория и в README), голый стем под замок не идет. Но полное имя
