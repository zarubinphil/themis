#!/usr/bin/env python3
"""stage7_spec.py — приёмка этапа 7 «три CLI». Пишет КООРДИНАТОР, не исполнитель.
...
Честная граница. Настоящий вызов Codex или Kimi — отправка наружу, и делает её владелец,
а не приёмка: замер и пилот из плана здесь не исполняются. Проверяется то, что решает
судьбу тайны доверителя: что именно уходит за границу процесса и что остаётся, когда
чужой CLI отказал. Подставной CLI для этого лучше настоящего — он показывает своё
окружение целиком.
...
PROBE_CONTRACT = """  scripts/cli_probe.py — доступность чужого CLI, пять исходов вместо «работает/нет»
    ...
    иначе 1. Исходы:
      ...
      timeout    — не ответил за отведённое время
    Кеш (--cache): повторная проба возвращает cached=true. Отказ по квоте БЕЗ даты
    сброса протухает через 5 часов — запись несёт `until`, и после него проба идёт
        ...
        if d:
            ...
            if not until:
                ...
                try:
                    ...
                    posle = {}
                if posle.get("cached"):
                    fails.append(("cli_probe.py", "после срока отказ всё ещё из кеша — "
                                                  "квота восстановилась, а мы её не видим"))
...
HERMETIC_CONTRACT = """  scripts/foreign_cli.py — вызов чужого CLI за границей процесса
    ...
    Порядок обязателен и fail-closed:
      ...
      2. рабочий каталог — временный, и в нём ТОЛЬКО обезличенный файл: ни дела,
      ...
      4. успех по трём сигналам: код 0, ответ непуст, в ответе нет маркеров отказа;
    ...
    либо кладётся в --out. В каталог дела не пишется НИЧЕГО ни при каком исходе."""
...
def check_hermetic():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Подставной CLI записывает всё, что ему досталось: каталог, окружение, stdin.
...
''')
        ...
        for k in env_keys:
            if k.startswith("THEMIS_") or "TOKEN" in k or "API_KEY" in k or "SECRET" in k:
                fails.append(("foreign_cli.py", f"наш секрет утёк в окружение чужого CLI: {k}"))
                break
        ...
        formy = [
            ...
            ("учреждение ребёнка",
             "Ребёнок 8 лет учится в гимназии № 7 г. Казани, живёт с матерью.\n", "гимнази"),
            ("СНИЛС без метки",
        ...
        if log.is_file():
            ...
            fails.append(("foreign_cli.py", "журнал отправок не ведётся"))
    return fails
...
OTKAZ_CONTRACT = """  scripts/foreign_cli.py — отказ
    Чужой CLI отказал (код не 0, пустой ответ, таймаут) либо обезличивание не прошло:
      ...
      · файл --out НЕ создаётся: половина ответа хуже отсутствия;
...
def check_otkaz():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        gryaznyy.write_text("Дело ведёт Сидоров Пётр, паспорт 1234 567890.\n", encoding="utf-8")
        ...
        for cmd, src, why in ((padayushchiy, vopros, "чужой CLI упал"),
            ...
            if out_file.exists():
                fails.append(("foreign_cli.py", f"{why}: файл ответа всё равно создан"))
...
def main():
    ap = argparse.ArgumentParser(description="Приёмка этапа 7 (пишет координатор).")
