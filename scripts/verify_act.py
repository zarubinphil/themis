...
SUBJECT_HINTS = {
    ...
    "осужден": "уголовное дело",
    "подсудим": "уголовное дело",
    "обвиняем": "уголовное дело",
}

# Предмет опознается по ЧАСТОТЕ подсказки, а не по первому вхождению в текст.
# Прецедент 21.08.2026: страницы публикаторов несут навигацию сайта («Административное
# право», «Налоги» в меню), и первое совпадение подстроки ловило именно ее — три
# кассационных определения ВС РФ по уголовным делам подряд получили предмет
# «административное дело», а охотник вынужден был опровергать прибор ручным чтением.
# Неверный предмет хуже пустого: он заставляет отбрасывать годный акт.
#
# Порог разный, потому что подсказки разной надежности. Многословная фраза
# («раздел совместно нажитого») в меню сайта не встречается — ей хватает одного
# вхождения. Односложный корень («административн», «уголовн») стоит в навигации
# почти каждой страницы публикатора — ему нужна частота, какую дает только текст
# самого акта.
NOISY_HINTS = ("административн", "налог", "трудов", "уголовн",
               "осужден", "подсудим", "обвиняем")
NOISY_MIN_HITS = 3


# Официальный публикатор нормативных актов. Адрес документа — по eoNumber, номеру
# электронного опубликования: 0001 + ГГГГММДД публикации + четырехзначный порядковый.
#
...
# Обратный случай хуже: несуществующий eoNumber отдает HTTP 404 страницей в 12,5 КБ —
    ...
    if not title:
        return {**base, "status": "НЕ ПОДТВЕРЖДЕН",
                "note": "страница без заголовка — идентификатора нет у публикатора "
                        "(HTTP 404 отдается полноразмерной страницей)"}
...
def fetch(url: str, dest: str) -> str:
    ...
    if size <= MIN_PAGE_BYTES:
        os.remove(tmp)
        # fetch_url.sh отдает !=0, когда не смог достать страницу вообще
...
def find_act_date(text: str, num: str) -> str | None:
    ...
    только по позиции: дата акта идет в связке «от <дата> № <номер>».
    """
...
def analyze(text: str, num: str, claimed_date: str | None = None) -> dict:
    ...
    for k, v in SUBJECT_HINTS.items():
        n = low.count(k)
        if n < (NOISY_MIN_HITS if k in NOISY_HINTS else 1):
            continue
        if n > best:
            best, subj = n, v
...
def verify(num: str, claimed_date: str | None = None, url: str | None = None) -> dict:
    """Проверяет акт. Пустой результат = 'не подтвержден', НЕ 'не существует'."""
...
def demo() -> None:
    ...
    assert r["act_date"] == "26.03.2019", f"дата акта берется после «от»: {r}"
    ...
    # Несуществующий eoNumber отдает 404 полноразмерной страницей без заголовка.
    # Диагноз обязан отличаться от «заголовок есть, но реквизитов в нем нет»:
    # первое — идентификатора не существует, второе — сменилась верстка публикатора.
    empty = analyze_eo("", "420-ФЗ", "30.11.2024")
    assert empty["status"] == "НЕ ПОДТВЕРЖДЕН" and "без заголовка" in empty["note"], empty
    # Заголовок без пары «от … №» не дает сверить реквизит — подтверждать нечем.
