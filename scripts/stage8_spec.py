#!/usr/bin/env python3
"""stage8_spec.py — приемка этапа 8 «бот Фемида». Пишет КООРДИНАТОР, не исполнитель.

Инвариант роя: generator ≠ verifier. Контракт задан снаружи, проверка — черным ящиком:
...
Честная граница. Настоящий Telegram здесь не дергается ни разу: первый живой запуск —
действие наружу, его делает владелец. Подставной сервер на 127.0.0.1 для приемки лучше
настоящего: он записывает КАЖДОЕ обращение бота и показывает, что именно ушло бы в чат.

Почему так строго. Все, что попало в Telegram, разглашено Telegram — это чужой сервер,
и адвокатская тайна (ст. 8 ФЗ № 63-ФЗ) на нем не действует. Значит, бот — пульт:
...
Восемь работ:
  ...
  6. ссылка на документ ведет внутрь приватной сети, токена в себе не несет и снаружи
     не открывается;
...
# Сеть наружу закрыта наглухо, петля разрешена: подставной Telegram живет на 127.0.0.1.
...
# и красил бы приемку на ровном месте.
    ...
    def __init__(self, updates):
        ...
        class H(BaseHTTPRequestHandler):
            ...
            def _rabota(self):
                ...
                # хвост ее не ломал, и проба на отказ доступа ничего не проверяла.
...
SECRET_CONTRACT = """  scripts/themis_bot.py — бот-пульт. Секрет живет ТОЛЬКО в окружении.
    ...
    Имя переменной с токеном берется из конфига (bot.token_env), значение — из окружения.
    ...
    формулировки отказов меняются, и однажды в деталь затечет строка с адресом запроса."""
...
ACCESS_CONTRACT = """  scripts/themis_bot.py — whitelist единственного chat_id.
    ...
    · бот работает опросом (getUpdates) и НИКОГДА не зовет setWebhook: сервер не открывает
      входящий порт;
...
def check_access():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Четвертое — подмена отправителя: чат владельца, а пишет чужой (так
...
RECH_CONTRACT = """  scripts/themis_bot.py — что бот вообще умеет сказать.
    --templates --json        {"fixture": {"pd": {...}, "safe": {...}},
                              ...
                              то, что бот называть обязан (дата, время, счет дел, ссылка);
                              приемка сама проверяет, что объявленное безопасным
    ...
    (disable_web_page_preview): увидев ссылку, сервер Telegram идет по ней САМ,
...
UTECHKI = ["Иванова Мария Петровна подала иск.",
           ...
           "Паспорт 9203 456789 приобщен к материалам.",
           # Проба 19.08.2026: короткие формы суммы, мимо которых прошла первая
           # редакция сторожа. Сумма выдает дело не хуже номера.
...
SSYLKA_CONTRACT = """  Документ забирается по ссылке ВНУТРЬ приватной сети, не через Telegram.
    ...
    · ссылка не называет ни дело, ни доверителя: путь заменен непрозрачным идентификатором
      (`pii_gate --residual` по самой ссылке дает 0);
    · ссылка выдается ТОЛЬКО на путь внутри cases/: «/etc/passwd» и «../../» — отказ
      с ненулевым кодом, а не запись в карту ссылок;
    · подмененная карта ссылок не открывает файл вне дел: панель проверяет путь сама;
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            if not wait_port(port):
                return fails + [("cockpit/app.py", "панель не поднялась за отведенное время")]
...
TISHINA_CONTRACT = """  Одно сообщение на одно событие; сказать нечего — бот молчит.
    scripts/themis_bot.py --notify-file ФАЙЛ --api-base URL
      ...
      · у каждого события есть свой производитель, а не только шаблон в корпусе:
        ...
        РОВНО одно сообщение; кривой аргумент дает отказ без отправки и без
        трассировки. Шаблон, который ничем не отправляется, — мертвая речь: приемка
...
AVATAR_CONTRACT = """  scripts/bot_avatar.py — изображение Фемиды для BotFather.
    --out ФАЙЛ [--size N]     кладет PNG не меньше 512×512 и печатает путь; код 0.
...
SERVE_CONTRACT = """  scripts/themis_bot.py --serve [--cycles N] --api-base URL — то, что владелец
    ...
    · --cycles N ограничивает число проходов (запуск по расписанию и приемка)."""
...
CHECKS = [
    ...
    ("ссылка ведет внутрь и снаружи не открывается", check_ssylka, SSYLKA_CONTRACT),
...
def selftest():
    ...
    try:
        ...
        SCRIPTS = saved
    # Подставной Telegram обязан работать сам по себе, иначе приемка меряет себя.
    with FakeTelegram([upd(1, OWNER_CHAT, "/status")]) as tg:
        req = urllib.request.Request(f"{tg.base}/bot{FAKE_TOKEN}/getUpdates")
        with urllib.request.urlopen(req, timeout=10) as r:
            d = json.loads(r.read().decode())
        assert d["result"][0]["message"]["chat"]["id"] == int(OWNER_CHAT), "подставной Telegram врет"
    ...
    assert not TOKEN_SHAPE.search(Path(__file__).read_text(encoding="utf-8")), \
        "сама приемка содержит строку формы токена"
    print("selftest: приемка краснеет на отсутствующих приборах, подставной Telegram честен — ок")
    return 0


def main():
    ap = argparse.ArgumentParser(description="Приемка этапа 8 (пишет координатор).")
