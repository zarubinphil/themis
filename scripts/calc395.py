...
ЗАЧЕМ. Расчёт по ст. 395 в проекте делался руками — то есть с ошибками, и каждая
...
def fetch_key_rates(since: dt.date, until: dt.date) -> list[tuple[dt.date, float]]:
    ...
    try:
        ...
        print(f"ЦБ РФ недоступен ({e}) — расчёт невозможен, повторить позже", file=sys.stderr)
        return []
...
def daily_series(rates: list[tuple[dt.date, float]], since: dt.date,
    ...
    ЦБ отдаёт ставку только по рабочим дням, но действует она непрерывно — до
    решения Совета директоров об изменении. Прямой разбор ответа терял субботы,
    воскресенья и праздники: на периоде 15.03.2025-04.08.2026 из расчёта выпадало
...
def calc(debt: float, since: dt.date, until: dt.date,
    ...
    """Проценты по периодам. Возвращает (строки расчёта, итог)."""
    rows, total = [], 0.0
    for a, b, rate in rate_periods(rates, since, until):
        # Период может пересекать границу года: число дней в году берётся по году,
...
def money_words(amount: float) -> str:
    """Сумма прописью в формате процессуального документа."""
...
def money_full(amount: float) -> str:
    """«327 120,50 (триста двадцать семь тысяч сто двадцать рублей 50 копеек)»."""
...
def selftest() -> int:
    ...
    checks = [
        ...
        ("периоды свёрнуты по смене ставки",
        ...
        ("ставка на начало периода берётся из более ранней даты",
        ...
        ("день уплаты включён", rows_y[0]["дней"] == 365),
        ...
        ("копейки склоняются", money_words(1.02).endswith("02 копейки")),
        ("ноль копеек", money_words(5.0).endswith("00 копеек")),
        ("дробная часть не теряется",
         money_words(327_120.50) == "триста двадцать семь тысяч сто двадцать рублей 50 копеек"),
        ("округление копеек вверх", money_words(0.005).endswith("01 копейка")),
...
def main() -> int:
    ...
    # Берём с запасом назад: ставку на первый день просрочки надо откуда-то знать,
    ...
    if not rows:
        print("ЦБ не дал ставок на этот период — расчёт не выполнен", file=sys.stderr)
        return 1
    ...
    if missing:
        print(f"\n⚠ ЦБ не дал ставку на {len(missing)} дн. периода — эти дни в расчёт "
