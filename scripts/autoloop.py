...
РЕЛЬСЫ (жестко в коде, не в прозе задания):
  · автономно исполняются только этапы 1, 2 и 5 — они покрыты приборами и не трогают
    ...
    владельца перед прогоном, даже при зеленом бэкапе;
  ...
  · окружение замораживается так же: установка пакетов автономно запрещена ВСЕГДА —
    цикл встает и ждет владельца;
  · итерация не закрывается без зеленого `loop_gate.py`.

Журнал — `.autoloop/journal.jsonl` (append-only), утренний отчет — `.autoloop/REPORT.md`.

Выход: 0 — цель достигнута; 1 — остановлен сторожем (причина в отчете);
...
# Этапы, покрытые приборами и не трогающие данные дел. Все прочее — руками.
# Этап 9 «замкнуть контур» легализован 19.08.2026 ТОЛЬКО в приборной части:
# правки scripts/, приемок, гейтов и документации. Владельческая часть этапа
...
def run(argv, cwd=ROOT, timeout=1800, env=None):
    ...
    stdin закрыт наглухо: человеческий гейт живет только у координатора. Роль,
...
# папки, Word кладет замок рядом с открытым документом. Материалами дела они не
...
# обиходе, останавливает работу вместо того, чтобы ее защищать.
...
# дважды вставал со словами «ТРОНУТЫ ДАННЫЕ ДЕЛ» при зеленом гейте: сравнение
# снимков 11 669 файлов дало ровно один измененный — _session_history.txt,
# 20032 -> 20064 байта. Сторож, кричащий без причины, учит игнорировать себя,
# а охраняет он 20 ГБ первички.
# Исключение УЗКОЕ и по МЕСТУ, не по имени: журнал признается служебным только
...
# круг 8). Прочие файлы с подчеркивания (_client.md, _case.md) отпечаток двигают.
...
def _dir_signature(h, path):
    """Подмешать имена прямых детей каталога: установленный пакет — новый ребенок.
    Каталога нет — тишина (еще не создан менеджером), а не срыв отпечатка."""
    try:
...
def env_fingerprint():
    ...
    Корень берется из переменной менеджера (NPM_CONFIG_PREFIX, GEM_HOME,
...
# ── Приемка конфигурации: fail-closed ────────────────────────────────────────
...
def _worktree_path(name, root=ROOT):
    ...
    STATE_DIR: приемка и чужая машина гоняют прибор с иным корнем, а путь от
    STATE_DIR увел бы рабочие копии в боевое дерево."""
...
def _is_worktree(path):
    ...
    Обычный каталог под .autoloop/worktrees git отнес бы к ОСНОВНОМУ репозиторию:
    `git reset --hard`, запущенный в нем, снес бы незакоммиченную работу
    координатора, `checkout -B` увел бы основное дерево на ветку роли. Факт
    проверяется git-ом: у настоящей рабочей копии ее собственный toplevel равен ей
...
def worktree_add(name, root=ROOT):
    ...
    файлы, а `cases/` под .gitignore — чужой CLI материалов дел в своем
    рабочем каталоге не видит вовсе. Перенос работы: роль коммитит в свою ветку,
    координатор забирает ее мержем (`worktree_merge`) — правки не теряются вместе
    ...
    # ударил бы по ОСНОВНОМУ дереву. Настоящую копию сюда не заносит: ее ловит ветка
...
def worktree_merge(name, root=ROOT):
    """Забрать коммиты роли из ее ветки в основное дерево. Конфликт — не катастрофа:
    мерж откатывается, роль теряет итерацию, причина остается в журнале.
    ...
    работа роли осталась в worktree и стерлась бы обновлением копии). ПД-сторож
...
def write_report(cfg, runlog, stop_reason, root=ROOT):
    """Утренний отчет: что сделано, чем доказано, где остановился и почему."""
    os.makedirs(os.path.join(root, ".autoloop"), exist_ok=True)
    ...
    lines = [
        ...
        "",
        f"Этап {cfg['stage']} · итераций {len(runlog)} · зеленых {len(green)}",
    ...
    for r in runlog:
        lines.append(f"| {r['iteration']} | {'зеленый' if r['green'] else 'красный'} | "
    ...
    lines += ["", "## Чем доказано", "",
              "Каждая зеленая итерация закрыта кодом возврата `scripts/loop_gate.py`: "
              "компиляция, `--selftest` затронутых приборов, smoke на `ivanov-ivan`, ПД-сторож. "
              "Вердикт модели пассом не считается.", "",
              "Журнал прогона: `.autoloop/journal.jsonl` — по нему отчет воспроизводится целиком.",
              ""]
...
def replay(root=ROOT):
    """Пересобрать отчет из журнала — прогон мог оборваться вместе с процессом.

    Отчет обязан быть воспроизводим с диска: если он существует только в памяти
    упавшего процесса, утром доказывать нечем.
    """
    path = os.path.join(root, ".autoloop", "journal.jsonl")
    if not os.path.isfile(path):
        return None, "журнала нет"
    cfg, runlog, stop = None, [], "журнал оборван — прогон не дошел до остановки"
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            if ev == "start":
                ...
                runlog, stop = [], "журнал оборван — прогон не дошел до остановки"
            elif ev == "gate":
...
def loop(cfg, root=ROOT, dry=False):
    ...
    for it in range(1, int(guards["max_iterations"]) + 1):
        ...
        def podgotovit(role):
            ...
            argv = [x.replace("{brief}", brief).replace("{python}", sys.executable)
                    for x in role["argv"]]
            cwd = root
            # Изоляция — ИНВАРИАНТ, а не флаг `parallel`: рабочая копия выдается
        ...
        # Волна параллельных ролей идет ОДНОВРЕМЕННО. Раньше флаг `parallel` только
        ...
        # гоняем (иначе бюджет все равно переехали бы на роль и/или гейт).
        ...
        # Зеленый гейт при тронутых делах или установленном пакете — это провал,
        # а не успех. Раньше сторожа стояли ПОСЛЕ раннего выхода по зеленому, и роль,
        ...
        if green:
            stop = "цель достигнута — гейт зеленый"
            break
    ...
    # Рабочая копия выдается КАЖДОЙ роли (изоляция — инвариант), значит и снимается
...
def selftest():
    ...
    assert not broken(stage="1") and not broken(stage="2"), "разрешенный этап отвергнут"
    ...
    assert "провал" in brief and base["roles"][1]["name"] not in brief.split("РОЛЬ")[0], \
        "в задание роли протек боковой обмен"
    ...
    with tempfile.TemporaryDirectory(prefix="autoloop-wt-") as tmp:
        ...
        try:
            ...
            assert os.path.isfile(os.path.join(tmp, "c.txt")), \
                "автокоммит прошел, а файла в main нет"
            assert os.path.isfile(os.path.join(tmp, "b.txt")), \
                "мерж прошел, а файла роли в main нет"
    ...
    with tempfile.TemporaryDirectory(prefix="autoloop-par-") as tmp:
        ...
        assert proshlo < 2.5, (f"три роли по секунде заняли {proshlo:.1f} с — волна идет "
...
def main():
    ...
    ap.add_argument("--replay", action="store_true",
                    help="пересобрать утренний отчет из журнала (процесс мог упасть)")
    ...
    if a.replay:
        report, stop = replay()
        if report is None:
            print(f"отчет не пересобран: {stop}", file=sys.stderr)
            return 1
        print(f"отчет пересобран из журнала: {os.path.relpath(report, ROOT)}\n  остановка: {stop}")
        return 0
    if not a.config:
        ap.print_help()
        return 2
    ...
    # может быть зеленой, а прогон — проваленным.
    success = stop.startswith("цель достигнута")
    print(f"\n{'✓' if success else '⛔'} остановлен: {stop}")
    print(f"  итераций: {len(runlog)} · отчет: {os.path.relpath(report, ROOT)}")
