...
РЕЖИМЫ:
  ...
  --missing                 догрузить ТОЛЬКО отсутствующее (дешево, дозагрузка)
...
# chapter_scope — если задан, в файл идут статьи ТОЛЬКО из этой главы (и ее
...
# берет из материалов дела. Данных доверителя в таком URL быть не может по
...
# дословное цитирование закона. Сторож остается на месте для любого чужого
...
def http_get(url: str, cache_key: str) -> bytes | None:
    ...
    зеленым: мертвый канал давал строку на stderr, в FAILURES не попадал,
    report_failures() возвращал 0, а legal-corpus-monthly.sh писал в лог
    «обновлено 0, ошибок 0» и завершался нулем.
    """
...
def split_into_punkty(text: str) -> list[tuple[str, str]]:
    ...
    ПОТЕРЯ ТЕКСТА, найденная аудитом 03.08.2026. Цикл шел `range(1, len(parts), 2)`
    и не забирал `parts[0]` — все, что стоит МЕЖДУ «постановляет:» и первым
    ...
    пересчет 325 кешированных страниц: потеряно 230 552 из 8 997 326 знаков (2,56 %),
...
def demo() -> None:
    ...
    # Мертвый канал обязан попадать в FAILURES, а не только на stderr: иначе
    # report_failures() вернет 0 и месячный прогон завершится «успешно».
    before = len(FAILURES)
    ...
    assert len(FAILURES) == before + 1, "мертвый канал не попал в FAILURES"
    ...
    print("demo: извлечение статьи, метаданные, фильтр главы, разбивка пунктов, "
          "проверка ответа и учет провалов — корректны")
...
def main() -> int:
    ...
    if a.missing:
        ...
        # он мог начать работать сегодня), а все остальное доезжает следом.
