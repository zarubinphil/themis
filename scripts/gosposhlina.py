...
Шкалу берем разбором ст. 333.19 (СОЮ, мировые судьи, ВС РФ) и ст. 333.21 (арбитраж)
...
`update_legal_corpus.py` и несет дату редакции, которую скрипт печатает рядом с суммой.

Отдельная причина существовать: колонка. Для физлица и для организации пошлина по
неимущественному иску отличается в разы, и ее уже путали на боевом деле (урок
...
def main() -> int:
    ...
    if a.prikaz:
        ...
        if floor and total < floor:
            note += (f" Применен минимум подп. 3 п. 1 ст. {art_num}: "
...
"""
    ...
    checks = [
        ...
        # Спецставки: подпункт дает прямую сумму, шкала цены иска не применяется.
        ...
        ("спецставка не берется из соседнего подпункта",
         flat_item_rate(fixture, "5") != flat_item_rate(fixture, "8")),
        ("несуществующая спецставка не выдумывается", flat_item_rate(fixture, "97") == 0),
        ("шкала цены иска не выдается за плоскую ставку", flat_item_rate(fixture, "1") == 0),
        ("процентная ставка приказа не выдается за сумму", flat_item_rate(fixture, "2") == 0),
        ("подпункт без своей суммы не берет ее у соседнего",
         flat_item_rate(fixture, "11") == 0),
        ("ставка подпункта с двумя суммами берется первой (подп. 16)",
    ...
    if article("333.19"):
        checks += [
            ...
            # Потолок верхней ступени арбитража берется из нормы, а не из головы:
