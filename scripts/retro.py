...
def headline(text: str, limit: int = 80) -> str:
    ...
    вид правила. Берём первую мысль до разделителя (→, точка, точка с запятой,
...
def main() -> int:
    ...
    if a.lesson is not None and not a.lesson.strip():
        ...
        # записи, журнал не остаётся полуписаным.
...
def selftest() -> int:
    ...
    checks = [
        ...
        ("урок без разделителей всё равно даёт заголовок",
