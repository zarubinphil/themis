...
def main():
    ...
    # Каталог рендера — кеш, а не содержимое дела: страницы восстанавливаются из
    # первички за секунды, а в дереве дел лежат вечно и уезжают в бэкап наравне
    # с доказательствами (замер 19.08.2026: 487 картинок кухни). Отказ ДО работы,
    # чтобы каталог не появился и рендер не начался.
    if a.render_dir and "cases" in os.path.abspath(a.render_dir).replace("\\", "/").split("/"):
        print("ERROR: --render-dir ведёт под cases/ — рендеры внутри дела запрещены. "
              "Каталог кеша: --render-dir /tmp/{дело}/{имя}", file=sys.stderr)
        sys.exit(2)
