...
`.agent/context/` дела и умирает вместе с ним, а следующая охота по той же теме идет
заново. Скрипт вычитывает ссылки на судебные акты из всех дел regex-ом ($0, без
модели), вычитает то, что уже есть в `knowledge/practice_index.md`, и выдает
кандидатов с цитатой-контекстом. Модели остается правовая оценка, не поиск.
...
PATTERNS = {
    ...
    # № 6-П соответствовал пяти датам, № 35-П — трем, № 9-П, 16-П и 18-П — двум
...
def sync_header(index_path: str) -> list[str]:
    """Счетчики шапки и оглавления — с диска. Шапка обещала 101 запись при 82 на диске,
    ...
    # счетчики по категориям в таблице «Содержание»
    per: dict[str, int] = {}
    current = None
...
def to_markdown(items: list[tuple[str, dict]]) -> str:
    ...
    for key, rec in items:
        ...
        # а контекст берется из материалов дела. Обещание «без ПД» должно быть правдой.
...
def selftest() -> int:
    ...
    open(os.path.join(ctx, "practice.md"), "w", encoding="utf-8").write(
        ...
        "и дело А65-12345/2024. Еще дело А65-99999/2024 упомянуто один раз. "
    ...
    checks = [
        ("уже внесенный Пленум не предлагается", "Постановление Пленума ВС РФ от 23.06.2015 № 25" not in cands),
        ("внесенное определение ВС не предлагается повторно", "81-КГ19-20" not in cands),
        ...
        ("одиночный номер производства в общую базу не идет",
         "А65-99999/2024" not in cands),
        ("номер, встреченный в двух делах, остается прецедентом",
        ...
        # Подстрочное поглощение: 81-КГ19-2 внесен, 81-КГ19-20 — другой акт и обязан
        # остаться кандидатом. Раньше он молча исчезал.
        # В базе лежит 81-КГ19-20; 81-КГ19-2 — ДРУГОЙ акт и обязан остаться кандидатом.
        # Подстрочная проверка находила его внутри внесенного и молча выбрасывала.
        ("короткий номер не поглощается длинным внесенным", "81-КГ19-2" in cands),
        ("номер собственного дела не идет в кандидаты", "2-777/2026" not in cands),
        ("чужой акт из того же файла остается", "5-КГ24-9-К2" in cands),
        ("выгрузка не содержит путей дел",
         "ivanov" not in to_markdown(list(cands.items()))),
        ("расхождение счетчика записей поймано", any("обещает 5" in p for p in aud)),
...
def main() -> int:
    ...
    ap.add_argument("--sync-header", action="store_true",
                    help="привести счетчики шапки и оглавления к тому, что на диске")
    ...
    if a.sync_header:
        for line in sync_header(a.index) or ["счетчики уже сходятся"]:
            print(f"  {line}")
        return 0
    ...
    if len(items) > a.top:
        print(f"\n… и еще {len(items) - a.top}. Полный список: --md FILE")
    # Находка — это работа, а не справка: код 0 при 402 несобранных актах читается
    # вызывающим скриптом как «все в порядке».
    return 1 if items else 0
